...
class Plotter():
    def __init__(self, ground_info, gt=None):
        self.RES_PATH = 'survey/res/v0.1/'
        self.STATION, self.gt_pos = gt
...
# TODO: Represent rssi with better granularity
    ...
    def _draw_beacon(draw:ID.ImageDraw, b): # b (beacon_x, beacon_y, rssi)
        if b is None or b[2] == 0:
            draw.ellipse((b[0]-12,b[1]-12,b[0]+12,b[1]+12), 'darkcyan')
             ...
             draw.ellipse((b[0]-30,b[1]-30,b[0]+30,b[1]+30), 'darkcyan')
             draw.text((b[0],b[1]), str(round(b[2])), 'white', ImageFont.truetype('config/arial.ttf', 36), anchor='mm')    
        else:
            draw.ellipse((b[0]-30,b[1]-30,b[0]+30,b[1]+30), 'blue')
            draw.text((b[0],b[1]), str(round(b[2])), 'white', ImageFont.truetype('config/arial.ttf', 36), anchor='mm')
    ...
    def draw_posterior(self, pf, beacon_batch=None, t:int|None=None): # t=0: initialize
        ...
        # Draw ground truth
        gt = self.point_pos_in_img(self.gt_pos)
        result_log += f'Error: {distance(self.gt_pos, c)}m; '
        draw.ellipse((gt[0]-40, gt[1]-40, gt[0]+40, gt[1]+40), fill='yellow', outline='blue', width=8)
        # Draw swarm
        ...
        r *= self.SCALE    
        draw.ellipse((c[0]-r,c[1]-r,c[0]+r,c[1]+r), fill=None, outline='red', width=10)
